...
3. If the tool is not available, the board MCP server is not loaded in this session - the plugin is not enabled here, or the `board` binary is not built. Say which by checking whether `${CLAUDE_PLUGIN_ROOT}/board/board.sh --help` runs, and stop. Do not start `board serve` from Bash as a substitute: a server outside the MCP process outlives the session and answers to nobody.
