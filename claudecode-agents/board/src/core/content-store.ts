...
// The config watcher below was `src/utils/config-watcher.ts` upstream. Its only caller is
// this store, so it lives here now that the shared helper module is gone.
...
function configWatcherDelay(ms: number): Promise<void> {
	return new Promise((resolve) => setTimeout(resolve, ms));
}

function watchConfigFile(
	filesystem: FileSystem,
	callbacks: { onConfigChanged?: (config: BacklogConfig | null) => void | Promise<void> },
): WatchHandle {
	...
	const notifyAfterStableRead = async (eventGeneration: number): Promise<void> => {
		for (let attempt = 0; attempt < CONFIG_READ_ATTEMPTS; attempt++) {
			await configWatcherDelay(attempt === 0 ? CONFIG_SETTLE_DELAY_MS : CONFIG_STABILITY_DELAY_MS);
			...
			try {
				...
				await configWatcherDelay(CONFIG_STABILITY_DELAY_MS);
				if (stopped || eventGeneration !== generation) {
					return;
				}
