...
18 September 2026: section 7's backing store is now the plugin's own copy of Backlog.md, a directory of markdown files in the memory tree at `board/`, per `docs/2026-09-17-backlog-board.md`. Section 7's body is left as the record and what it decides is unchanged: the five columns mean what they meant, the hooks still own every column move and no agent writes one, and "blocked by human" is still the one queue the human monitors. What changed is underneath - task files carried by the memory sync agent instead of GraphQL against a hosted tracker, items numbered `BD-12`, and the plugin shipping both the binary the hooks call and the MCP server three bodies carry. The Milestone 4 decision was taken early, ahead of the milestones before it, because the hooks were carrying a dependency the fleet did not control.
...
It also covers the plumbing. Where their memory lives, how they talk to Linear, what belongs in a skill versus CLAUDE.md, which plugins earn a place, and how the whole lot stays identical across your machines and Claude Code on the web while still tracking new models.
...
|---|---|---|---|---|---|---|---|
| `lead` | Plan, route, gate. Writes `docs/plans/<issue>.md` via the built-in Plan agent, picks who gets what, merges handoffs, holds the escalation policy. Not a subagent: set via `agent` in project settings | `opus` | n/a | full session, plus Linear MCP and rzem-memory MCP (read and write) | none | n/a | glossary, handoff, board |
...
| `spec-writer` | Turn a brain dump or board item into a spec: problem, non-goals, acceptance criteria, open questions. Interviews you before writing | `opus` | xhigh | Read, Grep, Glob, Write (docs/specs only), WebSearch, Linear MCP (read), rzem-memory MCP (read) | none | none | glossary, handoff, board, brainstorming |
...
## 7. Task tracking: Linear plus native tasks, nothing in between

Two layers only. Linear is the human-facing backlog and the visible board; Claude Code's native task list (`TaskCreate`, `blockedBy`, persistent via `CLAUDE_CODE_TASK_LIST_ID`, shared with Agent Teams) is the execution layer inside a session. Nothing sits between them. The mapping rule lives in the glossary: an item may spawn many tasks; a task never creates a board item on its own.

The structure is two databases. **Projects** holds bounded bodies of work, with Initiative as a select field so a goal spanning several projects has somewhere to live. **Tasks** holds the tracked units, related to a project, with Milestone as a field and sub-issues as a self-relation. One workspace, everything in it: home lab, the fleet itself, rzem.guru, Deloitte and client work, personal admin. The point of one queue is that you look in one place, and a board you have to mentally join to another board is not one place.

The board is a view on Tasks with five columns:
...
Wire Linear in with the connector, and scope the MCP to `spec-writer`, `fleet-steward` and the lead - the three that create and read items. Not to `coder`. This is the general rule and it's worth stating once: every MCP server's tool list is paid for on every turn of every agent that carries it, so use `disallowedTools` with `mcp__*` patterns on the agents that don't need one. Context budget is a real budget. rzem-memory is the deliberate exception - it goes to all ten (section 6) because cross-session recall is worth the tool list, and because it's your server and you control how wide that surface gets. Keep it narrow. Everything else, including Linear, stays scoped.

Note that hooks can't call MCP tools - they run as shell on the host - so the status writes hit Linear's GraphQL API directly with a personal API key. Section 12 covers where that key lives.

The git-native trackers (beads on Dolt, Backlog.md as markdown) are the third layer some people add so cloud sessions can query the graph without an MCP round trip. Skip for now. If you find the `blockedBy` graph in native tasks isn't enough across sessions, Backlog.md is the one to try because you can read it without a tool. task-master and BMAD are both too heavy for one person.
...
One consequence of section 7 worth writing down rather than discovering. The board is one workspace holding everything at full detail, Deloitte and client work included, and three agents carry the Linear MCP. That means client material is reachable by an LLM session, sits in a personal Linear workspace, and travels wherever that workspace's sharing settings allow. You've made that call deliberately and it's your board, but it belongs in this section rather than nowhere: check what the Linear connector's OAuth scope actually grants before wiring it, keep the workspace's guest and public-link defaults closed, and treat an issue you'd have to redact before showing a client as an issue that shouldn't be on the board in full.
...
Milestone 4: measure for a month, then decide about Backlog.md, Agent Teams for implementation, and whether Sonnet 5 can take over `coder` for routine issues.
