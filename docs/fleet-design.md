...
|---|---|---|---|---|---|---|
| `lead` | Plan, route, gate. Writes `docs/plans/<issue>.md` via the built-in Plan agent, picks who gets what, merges handoffs, holds the escalation policy. Not a subagent: set via `agent` in project settings | `opus` | high | the full session, which carries the board server and memory (read and write) | n/a | glossary, handoff, board |
...
| `spec-writer` | Turn a brain dump or board item into a spec: problem, non-goals, acceptance criteria, open questions. Interviews you before writing | `opus` | medium | Read, Grep, Glob, Write (`docs/specs/` only), WebSearch, board (read), memory (read) | - | glossary, handoff, board, brainstorming |
