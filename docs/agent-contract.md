...
`skills:` resolves a name from the plugin, the project and user scope alike, and a name that resolves nowhere is a silent no-op (check 6 of `migration-checklist`). As of 12 September 2026 every name a body preloads resolves: the plugin ships `glossary`, `handoff`, `board`, `migration-checklist`, `compound`, `run-article`, `looping` and `humanize`, and `brainstorming` resolves from the superpowers plugin. Forward references were stripped that day - a body naming a skill that does not exist tells the agent a procedure is loaded when it is not, which the opencode-agents port's findings document showed is worse than the body carrying the procedure inline. The roadmap for future skills (`using-memory`, `docwright`, the stack suite and the rest) lives in plan sections 8 and 15; when one ships, add it to the bodies that need it and update this paragraph.
...
Two scoping notes that go with the names. rzem-memory reaches all ten agents deliberately (plan section 6); every other server stays scoped, because an MCP server's tool list is paid for on every turn of every agent that carries it. And an agent body cannot set `mcpServers` of its own (plan section 9), so a server reaches an agent one of two ways: the plugin ships it in its own `.mcp.json`, which is how the board's server arrives and where a new fleet-owned server belongs, or it is a claude.ai connector the human has enabled. Either way the body only names it, under the identifier the table above records.
